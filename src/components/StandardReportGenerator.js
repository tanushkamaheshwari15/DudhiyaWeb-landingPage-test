...
const StandardReportGenerator = ({ variant = 'dashboard' }) => {
  const { t } = useLanguage();
  ...
  // Report types
  const reportTypes = [
    { id: 'full', name: t('fullReport'), icon: faFileAlt, description: t('viewCompleteCollectionReport') },
    { id: 'purchase', name: t('purchaseReportDayWise'), icon: faFileInvoice, description: t('viewCollectionsByDate') },
    { id: 'summary', name: t('purchaseSummarySupplierWise'), icon: faFileContract, description: t('viewCollectionsAsPerSupplier') },
    { id: 'bills', name: t('allSupplierBill'), icon: faFilePdf, description: t('viewComprehensiveSupplierBills') },
    { id: 'supplier_bills', name: t('supplierBills'), icon: faFilePdf, description: t('viewIndividualSupplierBills') }
  ];
  ...
  const getReportTypeString = (reportId) => {
    switch (reportId) {
      ...
      case 'purchase': return 'purchase_report';
      case 'summary': return 'summary_report';
      case 'bills': return 'milk_bills';
  ...
  const downloadReport = async () => {
    ...
    try {
      ...
      switch (selectedReport.id) {
        case 'purchase':
          response = await generatePurchaseReport(s, e);
          break;
        case 'summary':
          response = await generatePurchaseSummaryReport(s, e);
          break;
        case 'full':
          response = await generateFullReport(s, e);
          break;
        case 'bills':
          response = await generateFullCustomerReport(s, e);
          break;
        default:
      ...
      const type = getReportTypeString(selectedReport.id);
      link.setAttribute('download', `${dairyName}-${type}-${fromName}-${toName}-${year}.pdf`);
      ...
      setError(err.error || err.message || t('failedToGenerateReport'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const downloadSupplierReport = async () => {
    ...
    try {
      const s = formatDateForAPI(fromDate);
      const e = formatDateForAPI(toDate);
      const response = await generateCustomerReport(s, e, selectedSupplier.id);
      ...
      const toName = formatDateForFilename(toDate);
      const year = new Date(toDate).getFullYear();
      link.setAttribute('download', `${dairyName}-supplier_bills_${selectedSupplier.name}-${fromName}-${toName}-${year}.pdf`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(url);
  ...
  return (
    <div className="pro-rata-report-generator">
      {variant === 'inline' ? (
        <button
          className="filter-button pro-rata-button"
          onClick={openModal}
        >
          <FontAwesomeIcon icon={faFileAlt} />
          <span>{t('generateStandardReport')}</span>
        </button>
      ) : (
        <button
        ...
        <div className="modal-overlay">
          <div className="report-modal-content">
            <div className="report-modal-header">
              <h2>{showDateFilter ? `${t('generateReport')} ${selectedReport.name}` : t('selectReportType')}</h2>
            ...
            {showDateFilter ? (
              <div className="report-date-filter-container">
                <div className="back-button-container">
                  <button className="back-to-reports-button" onClick={backToReportSelection}>{t('backToReportSelection')}</button>
                </div>
                ...
                <div className="date-inputs-container">
                  <div className="date-input-group">
                    ...
                    <div className="date-input-wrapper" onClick={() => document.getElementById('std-from-date-input').showPicker()}>
                      {/* <FontAwesomeIcon icon={faCalendarAlt} className="date-icon" /> */}
                      <input id="std-from-date-input" type="date" className="date-input" value={fromDate} onChange={e => setFromDate(e.target.value)} />
                    </div>
                  </div>
                  <div className="date-input-group">
                    <label>{t('toDate')}</label>
                    <div className="date-input-wrapper" onClick={() => document.getElementById('std-to-date-input').showPicker()}>
                      {/* <FontAwesomeIcon icon={faCalendarAlt} className="date-icon" /> */}
                      <input id="std-to-date-input" type="date" className="date-input" value={toDate} onChange={e => setToDate(e.target.value)} />
                    </div>
                  </div>
                </div>
                ...
                <div className="report-actions">
                  <button className="secondary-button-generate" onClick={backToReportSelection} disabled={loading}>{t('cancel')}</button>
                  <button className="primary-button-generate" onClick={downloadReport} disabled={loading || !fromDate || !toDate}>
                    {loading ? (<><FontAwesomeIcon icon={faSpinner} className="fa-spin" /> {t('generating')}</>) : t('generateReport')}
                  </button>
                </div>
              </div>
            ) : (
              <div className="report-selection-container">
                <div className="report-types-grid">
                  {reportTypes.map(r => (
                    <div key={r.id} className="report-type-card" onClick={() => handleSelectReport(r)}>
                      <FontAwesomeIcon icon={r.icon} className="report-icon" />
                      <h3>{r.name}</h3>
                      <p>{r.description}</p>
                    </div>
                  ))}
                      ...
                      .map(supplier => (
                        <div key={supplier.id} className="supplier-card" onClick={() => selectSupplier(supplier)}>
                          <div className="supplier-info">
                            <h3>{supplier.name}</h3>
                            <p><strong>{t('supplierID')}:</strong> {supplier.customer_id}</p>
      ...
      {showSupplierDetailsModal && selectedSupplier && ReactDOM.createPortal(
        <div className="modal-overlay">
          <div className="report-modal-content">
            <div className="report-modal-header">
              <h2>{selectedSupplier.name} - {t('supplierBills')}</h2>
              ...
              <div className="supplier-actions-section">
                {isEditingSupplier ? (
                  ...
                  <>
                    ...
                    {/* <button className="view-pro-rata-btn" onClick={downloadProRataSupplierReport} disabled={loading}>
                      {loading ? (
                        <>
                          <FontAwesomeIcon icon={faSpinner} spin />
                        ...
                        <>
                          ...
                          <span>View Pro Rata Reports</span>
                        </>
                      )}
                    </button> */}
...
export default StandardReportGenerator;
