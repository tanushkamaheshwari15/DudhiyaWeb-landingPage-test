// Supplier Workflow Page Translations
class SupplierWorkflowTranslations {
    constructor() {
        this.translations = {};
    ...
    async loadTranslations() {
        try {
            ...
            for (const lang of languages) {
                try {
                    const response = await fetch(`translations/supplier-workflow/${lang}.json`);
                    if (response.ok) {
                        this.translations[lang] = await response.json();
                    } else {
                        console.warn(`Failed to load ${lang} translations for supplier workflow`);
                    }
                } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
    window.supplierWorkflowTranslations = new SupplierWorkflowTranslations();
});

// Make it available globally for manual language switching
window.switchSupplierWorkflowLanguage = (language) => {
    if (window.supplierWorkflowTranslations) {
        window.supplierWorkflowTranslations.setLanguage(language);
    }
};

// Export for global access to update language without triggering events (used after navbar load)
window.updateSupplierWorkflowLanguage = async (language) => {
    if (window.supplierWorkflowTranslations) {
        window.supplierWorkflowTranslations.currentLanguage = language;
        await window.supplierWorkflowTranslations.loadTranslations();
        window.supplierWorkflowTranslations.applyTranslations();
    }
};
